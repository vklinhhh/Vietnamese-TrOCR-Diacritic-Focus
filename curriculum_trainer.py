...
def train_vietnamese_ocr_with_curriculum(
    ...
    """
    Train Vietnamese OCR model with curriculum learning.
    ...
    while total_epochs_completed < epochs:
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {total_epochs_completed+1}, Stage {train_curriculum.current_stage+1}"):
            ...
            for pos in range(seq_length):
                ...
                diacritic_loss += ce_loss(outputs['diacritic_logits'][:, pos, :], 
            ...
            diacritic_weight = 0.5 + 0.5 * stage_ratio  # 0.5 -> 1.0
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validation"):
                ...
                # Always use full weights during validation (for consistent tracking)
...
def log_curriculum_learning_curves(stage_histories):
    ...
    for i, history in enumerate(stage_histories):
        ...
        # FIX: pass color and line style separately
    ...
    # FIX: Save figure to a temporary file instead of BytesIO
    import tempfile
